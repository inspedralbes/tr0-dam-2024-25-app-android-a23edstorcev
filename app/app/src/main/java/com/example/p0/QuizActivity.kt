...
import com.example.p0.ApiService
import com.example.p0.Question
import com.example.p0.R
import com.example.p0.ResultActivity
...
class QuizActivity : AppCompatActivity() {
    ...
    private fun showQuestion(index: Int) {
        ...
        radioButtonOption1.text = question.resposta[0]
        radioButtonOption2.text = question.resposta[1]
        radioButtonOption3.text = question.resposta[2]
        radioButtonOption4.text = question.resposta[3]

        btnNext.setOnClickListener {
            val selectedId = radioGroupOptions.checkedRadioButtonId
            val answerIndex = radioGroupOptions.indexOfChild(findViewById(selectedId))

            userAnswers.add(answerIndex)

            if (answerIndex == question.resposta.indexOf(question.resposta[0])) { // Cambia según tu lógica de respuesta correcta
                correctAnswers++
            }
